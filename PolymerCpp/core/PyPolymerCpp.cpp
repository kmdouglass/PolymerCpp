...
static PyObject * getWLCrgs(PyObject *self, PyObject *args)
{
	...
	int alsoBumped;
	double locPrecision;
    if (!PyArg_ParseTuple(args, "idddid", &numPaths, &pathLength,
    	&linDensity, &persisLength, &alsoBumped,
    	&locPrecision))
        return NULL;
...
static PyObject * getWLC(PyObject *self, PyObject *args)
{
	...
	int bumped;
	double locPrecision;
    if (!PyArg_ParseTuple(args, "dddid", &pathLength,
    	&linDensity, &persisLength, &bumped,
    	&locPrecision))
        return NULL;
...
// changed the type of chain from WLC to SAWLC.
{
	...
	double locPrecision;
	double linkDiameter;
    if (!PyArg_ParseTuple(args, "idddidd", &numPaths, &pathLength,
    	&linDensity, &persisLength, &alsoBumped,
...
// changed the type of chain from WLC to SAWLC.
{
	...
	double locPrecision;
	double linkDiameter;
    if (!PyArg_ParseTuple(args, "dddidd", &pathLength,
    	&linDensity, &persisLength, &bumped,
    	&locPrecision, &linkDiameter))
        return NULL;
